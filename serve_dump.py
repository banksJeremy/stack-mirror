...
def serve(db, port):
    from bottle import route, view, request, run, static_file, redirect, error
    ...
    def index():
        ...
        with db:
            cursor = db.cursor()
            cursor.execute("SELECT Id, Title, Score, ViewCount FROM Posts Where PostTypeId = 1 LIMIT 100")
    ...
    @view("views/question")
    def question(question_id, slug):
        question = None
        answers = []
        
        with db:
            cursor = db.cursor()
            cursor.execute("""SELECT Id, Title, Body, Score, ViewCount, ParentId
                              FROM Posts WHERE Id = ? OR ParentId = ?""",
                           [question_id, question_id])
            
            for post_id, title, body, score, views, parent in cursor:
                this = {"title": title, "body": body, "views": views, "score": score}
                
                if post_id == question_id:
                    if parent:
                        logger.warning("Answer ID used in question request.")
                        return error(404)
                    question = this
                else:
                    answers.append(this)
        ...
        if slug != proper_slug:
            ...
            return redirect("/questions/{0}/{1}".format(post_id, proper_slug), 301)
